...
bool LCD::configVendorCommands(const esp_panel_lcd_vendor_init_cmd_t init_cmd[], uint32_t init_cmd_size)
{
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(
        ...
        std::static_pointer_cast<BusRGB>(bus)->getConfig().use_control_panel, false,
...
bool LCD::begin()
{
...
#else
        auto rgb_config = std::static_pointer_cast<BusRGB>(bus)->getConfig().getRefreshPanelFullConfig();
    ...
    default:
        ...
        ESP_UTILS_CHECK_ERROR_RETURN(
            esp_lcd_panel_io_register_event_callbacks(bus->getControlPanelHandle(), &io_cb, &_interruption.data), false,
            "Register control panel event callback failed"
        );
        break;
    }
...
bool LCD::reset()
{
    ...
    if ((bus->getBasicAttributes().type == ESP_PANEL_BUS_TYPE_RGB) && !isOverState(State::BEGIN) &&
...
bool LCD::drawBitmap(uint16_t x_start, uint16_t y_start, uint16_t width, uint16_t height, const uint8_t *color_data)
{
    ...
    if ((bus->getBasicAttributes().type == ESP_PANEL_BUS_TYPE_RGB) && (_interruption.on_draw_bitmap_finish != nullptr)) {
...
bool LCD::setDisplayOnOff(bool enable_on)
{
    ...
    ESP_UTILS_CHECK_ERROR_RETURN(esp_lcd_panel_disp_on_off(refresh_panel, enable_on), false, "Set display on/off failed");
...
bool LCD::DSI_ColorBarPatternTest(DSI_ColorBarPattern pattern)
{
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(isOverState(State::BEGIN), false, "Not begun");
    ESP_UTILS_CHECK_FALSE_RETURN(
        bus->getBasicAttributes().type == ESP_PANEL_BUS_TYPE_MIPI_DSI, false, "Only valid for MIPI-DSI bus"
    );
...
int LCD::getFrameColorBits()
{
    ...
    switch (bus->getBasicAttributes().type) {
#if SOC_LCD_RGB_SUPPORTED
    case ESP_PANEL_BUS_TYPE_RGB: {
        auto rgb_config = std::static_pointer_cast<BusRGB>(bus)->getConfig().getRefreshPanelFullConfig();
...
#if SOC_MIPI_DSI_SUPPORTED
    case ESP_PANEL_BUS_TYPE_MIPI_DSI: {
        auto dpi_config = std::static_pointer_cast<BusDSI>(bus)->getConfig().getRefreshPanelFullConfig();
...
bool LCD::processDeviceOnInit(const BasicBusSpecificationMap &bus_specs)
{
    ...
    ESP_UTILS_CHECK_FALSE_RETURN(isBusValid(), false, "Invalid bus");
    ESP_UTILS_CHECK_FALSE_RETURN(bus->isOverState(Bus::State::BEGIN), false, "Bus is not begun");
...
// *INDENT-OFF*
    ...
    if ((bus->getBasicAttributes().type != ESP_PANEL_BUS_TYPE_RGB) || (bus->getControlPanelHandle() != nullptr)) {
...
// *INDENT-OFF*
    ...
    if (bus_type == ESP_PANEL_BUS_TYPE_RGB) {
        if ((std::static_pointer_cast<BusRGB>(bus)->getConfig().getRefreshPanelFullConfig()->disp_gpio_num == -1) && (
            (bus->getControlPanelHandle() == nullptr) || getConfig().getVendorFullConfig()->flags.enable_io_multiplex
            )) {
    ...
    case ESP_PANEL_BUS_TYPE_RGB:
        ...
        vendor_config.rgb_config = std::static_pointer_cast<BusRGB>(bus)->getConfig().getRefreshPanelFullConfig();
        break;
#endif
#if SOC_MIPI_DSI_SUPPORTED
    /* Retrieve MIPI DPI configuration from the bus and register it into the vendor configuration */
    case ESP_PANEL_BUS_TYPE_MIPI_DSI: {
        auto dsi_bus = std::static_pointer_cast<BusDSI>(bus);
...
IRAM_ATTR bool LCD::onDrawBitmapFinish(void *panel_io, void *edata, void *user_ctx)
{
    ...
    if (lcd_ptr->_interruption.on_draw_bitmap_finish != nullptr) {
        need_yield = lcd_ptr->_interruption.on_draw_bitmap_finish(lcd_ptr->_interruption.data.user_data) ? pdTRUE : need_yield;
    }
...
IRAM_ATTR bool LCD::onRefreshFinish(void *panel_io, void *edata, void *user_ctx)
{
    ...
    if (lcd_ptr->_interruption.on_refresh_finish != nullptr) {
        need_yield = lcd_ptr->_interruption.on_refresh_finish(lcd_ptr->_interruption.data.user_data) ? pdTRUE : need_yield;
    }
