...
public:
    ...
    /**
     * @brief Construct a ST7796 LCD device
     *
     ...
     */
    LCD_ST7796(Bus *bus, uint8_t color_bits, int rst_io = -1):
        LCD(bus, color_bits, rst_io, ATTRIBUTES_DEFAULT)
    {
    }

    /**
     * @brief Construct a new ST7796 LCD device
     *
     * @param bus    Pointer of panel bus
     * @param config LCD configuration
     */
    LCD_ST7796(std::shared_ptr<Bus> bus, const Config &config):
 ...
 *             Please use `esp_panel::drivers::LCD_ST7796` instead.
 *
 */
typedef esp_panel::drivers::LCD_ST7796 ESP_PanelLcd_ST7796 __attribute__((deprecated("Deprecated and will be removed in the next major version. Please use `esp_panel::drivers::LCD_ST7796` instead.")));
