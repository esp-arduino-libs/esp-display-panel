...
ESP32_Display_Panel 会根据 [esp_panel_drivers_conf.h](../esp_panel_drivers_conf.h) 文件来配置驱动的功能和参数，用户可以通过修改此文件中的宏定义来更新驱动的行为或默认参数。以使能用于调试的 LOG 输出为例，下面是修改后的 `esp_panel_drivers_conf.h` 文件的部分内容：
