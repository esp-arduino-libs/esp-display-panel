...
static void run_test(shared_ptr<LCD> lcd, bool use_config)
{
    ...
    if (lcd->getBasicAttributes().basic_bus_spec.isFunctionValid(LCD::BasicBusSpecification::FUNC_MIRROR_X)) {
        TEST_ASSERT_TRUE_MESSAGE(lcd->mirrorX(true), "LCD mirror X failed");
    }
    if (lcd->getBasicAttributes().basic_bus_spec.isFunctionValid(LCD::BasicBusSpecification::FUNC_MIRROR_Y)) {
